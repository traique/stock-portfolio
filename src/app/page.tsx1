...
import { Activity, BookOpen, Newspaper, RefreshCw, Sparkles, Trash2, X } from 'lucide-react';
...
function EmptyAiScan() {
  return (
    <div className="ab-empty" style={{ paddingTop: 24, paddingBottom: 24 }}>
      ...
      <div className="ab-empty-desc">
        Bấm <strong style={{ color: 'var(--text)' }}>QUÉT</strong> để AI phân tích kỹ thuật và gợi ý điểm mua/bán cho các mã trong danh sách.
      </div>
    </div>
  );
}
    ...
    <article style={QUOTE_ARTICLE_STYLE}>
      <div className="ab-row-between align-start" style={{ marginBottom: 10 }}>
        <div>
          ...
          <span className="ab-stock-badge">Cổ phiếu</span>
        </div>
        <button
          type="button"
...
function AiPickCard({ pick }: { pick: AiPick }) {
  return (
    <div style={{ padding: 14, border: '1px solid var(--border)', borderRadius: 16, background: 'var(--soft)' }}>
      ...
      <div style={{ color: 'var(--text)', fontSize: 12, marginBottom: 12, lineHeight: 1.55, opacity: 0.72 }}>
        {pick.reason}
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
        <div style={PICK_ENTRY_STYLE}>
          <div style={{ fontSize: 9, color: 'var(--subtle)', fontWeight: 800 }}>ENTRY</div>
          <div className="num-premium" style={{ fontWeight: 800, fontSize: 13, marginTop: 2, color: 'var(--text)' }}>
            {formatPrice(pick.entry)}
          </div>
        </div>
        <div style={{ ...PICK_TP_STYLE, background: 'var(--green-surface)', border: '1px solid var(--green-border)' }}>
        ...
        <div style={{ ...PICK_SL_STYLE, background: 'var(--red-surface)', border: '1px solid var(--red-border)' }}>
  ...
  return (
    <main className="ab-page">
      <div className="ab-shell">
        ...
        {/* --- VN-INDEX OVERVIEW --- */}
        {/* --- VN-INDEX compact single-row --- */}
        <section className="ab-premium-card">
          <div className="ab-vnindex-compact">
            {/* Main index */}
            <div className="ab-vnindex-compact-main">
              <span style={{ fontSize: 11, fontWeight: 800, color: 'var(--subtle)', display: 'flex', alignItems: 'center', gap: 5, letterSpacing: '0.06em' }}>
                <Activity size={12} color="var(--green)" /> VN-INDEX
              </span>
              <div className="num-premium" style={{ fontSize: 'clamp(22px, 3.5vw, 32px)', fontWeight: 800, lineHeight: 1, color: 'var(--text)' }}>
                {vnIndex ? formatPrice(vnIndex.price) : '--'}
              </div>
              <div className="num-premium" style={{ color: colorFor(vnIndex?.pct), fontWeight: 700, fontSize: 13 }}>
            ...
            {[
              ...
              <div key={stat.label} className="ab-vnindex-compact-stat">
                <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--subtle)' }}>{stat.label}</span>
                <div className="num-premium" style={{ color: stat.color, fontSize: 'clamp(18px, 2.5vw, 24px)', fontWeight: 800 }}>
                  {stat.value}
                </div>
              </div>
            ))}
          ...
          <section className="ab-premium-card" style={CARD_STYLE}>
            ...
            {loading ? (
              ...
              <EmptyWatchlist onAdd={() => {
                const el = document.querySelector<HTMLInputElement>('.ab-input');
                el?.focus();
              }} />
            ) : (
              <div className="ab-quote-grid" style={{ display: 'grid', gap: 12, gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  ...
                  {aiWatchlist.avoid.length > 0 && (
                    <div style={{
                      ...
                      background: 'var(--red-surface)',
                      border: '1px solid var(--red-border)',
                      fontSize: 12, color: 'var(--text)', lineHeight: 1.5, opacity: 0.9,
                    }}>
                  ...
                  topPositive.map(item => (
                    ...
                    >
                      <div style={{ minWidth: 0, flex: 1 }}>
                        ...
                        <div className="num-premium" style={{ fontSize: 12, color: 'var(--subtle)', marginTop: 2 }}>
                          {formatPrice(item.price)}
                        </div>
                      </div>
