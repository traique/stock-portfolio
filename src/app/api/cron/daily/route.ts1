...
async function fetchPrices(symbols: string[]): Promise<Record<string, number>> {
  ...
  try {
    ...
    const res = await fetch(
      `${base}/api/prices?symbols=${encodeURIComponent(symbols.join(','))}`,
      { cache: 'no-store' },
    );
    if (!res.ok) return {};
...
// Lấy thêm debug array (change/pct) dùng cho Telegram
async function fetchPricesWithDebug(symbols: string[]) {
  ...
  try {
    ...
    if (!res.ok) return { prices: {} as PriceMap, debug: [] };
    const data = await res.json();
    return {
      prices: (data?.prices ?? {}) as PriceMap,
      debug:  (data?.debug  ?? []) as Array<{ symbol: string; price: number; change: number; pct: number }>,
    };
  } catch {
