...
const querySchema = z.object({
  ...
  // Regex chặt: timeframe được nối thẳng vào URL upstream → không cho ký tự đặc biệt
  timeframe: z.string().trim().regex(/^[A-Za-z0-9_-]{1,8}$/).transform(s => s.toUpperCase()).optional().default('1D'),
});
