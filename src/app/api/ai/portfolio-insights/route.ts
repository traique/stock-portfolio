...
const TP_DEFAULT_MULT = 1.08;
const SL_DEFAULT_MULT = 0.95;
  ...
  return positions.map(pos => {
    ...
    return {
      ...
      technical: {
        ...
        momentumPct:   sig?.momentumPct   ?? 0,  // was momentum5dPct — fixed
...
function buildBaseActions(context: PortfolioContextItem[]): AiAction[] {
  return context.map(item => {
    ...
    const tp = Math.round((item.currentPrice * TP_DEFAULT_MULT) / 10) * 10;
    const sl = Math.round((item.currentPrice * SL_DEFAULT_MULT) / 10) * 10;
...
export async function POST(request: NextRequest) {
  ...
  const portfolioContext = buildPortfolioContext(positions, signals);
