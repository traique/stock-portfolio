...
import { aiPortfolioResponseSchema, enforceTpSl } from '@/lib/server/ai-schemas'; // ✨ Phase 0.2 — validate output + enforce TP/SL
import { buildAiCacheMeta, checkAiRateLimit, getAiCache, getRateLimitResetSeconds, setAiCache } from '@/lib/server/ai-cache';
// ✨ Phase 0 — đã bỏ `import { envServer } from '@/lib/env-server'` vì không dùng trong file này.
...
export async function POST(request: NextRequest) {
  ...
  if (!parsed.data.force_refresh) {
    const cached = await getAiCache<AiPortfolioResponse>(cacheKey);
    if (cached) {
      return NextResponse.json({ ...cached, ...buildAiCacheMeta(PORTFOLIO_AI_CACHE_TTL_MS) });
    }
  }

  // Rate limit — only applied when an actual AI call is needed (cache miss)
  if (!checkAiRateLimit(user.id)) {
    const retryAfter = getRateLimitResetSeconds(user.id);
    logger.warn('[portfolio-insights] Rate limit hit', { userId: user.id, retryAfter });
    return NextResponse.json(
  ...
  const portfolioRisk = calcPortfolioRisk(
    portfolioContext.map(item => ({
      ...
      value: item.positionValue, // ✨ FIX mismatch — cần cho field `basis` mới trong PortfolioRisk
    })),
  );
  ...
  const aiCallResult = await callAiWithFallback<AiPortfolioResponse>(
    ...
    fallback,
    aiPortfolioResponseSchema, // ✨ Phase 0.2 — validate JSON output; sai định dạng → tự fallback
  );

  // ✨ Phase 0.2 — enforce sl < giá vốn < tp cho TỪNG action (server quyết định, không tin LLM)
  const avgBuyMap = new Map(portfolioContext.map(item => [item.symbol, item.avgBuyPrice]));
  const safeActions = aiCallResult.data.actions.map(a =>
    enforceTpSl(a, avgBuyMap.get(a.symbol) ?? 0),
  );

  const finalResponse: AiPortfolioResponse = {
    ...aiCallResult.data,
    actions: safeActions, // ✨ Phase 0.2 — dùng actions đã enforce TP/SL
