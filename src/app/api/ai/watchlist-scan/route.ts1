...
import { watchlistResponseSchema, enforceTpSl } from '@/lib/server/ai-schemas';
...
export async function POST(request: NextRequest) {
  ...
  const safePicks = aiCallResult.data.picks.map(p =>
    enforceTpSl(p, currentPriceMap.get(p.symbol) ?? 0),
  );

  const finalResponse: WatchlistScanResponse = {
    ...aiCallResult.data,
    picks: safePicks, // ✨ dùng picks đã enforce tp/sl
