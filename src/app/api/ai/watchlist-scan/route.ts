...
// ✨ FIX (parity với portfolio-insights) — validate output LLM + chặn tp/sl vô lý trước khi trả về người dùng
import { watchlistResponseSchema, enforceTpSl, enforceAndClampSniperPoints } from '@/lib/server/ai-schemas';
...
export async function POST(request: NextRequest) {
  ...
  const aiCallResult = await callAiWithFallback<WatchlistScanResponse>(
    ...
    fallback,
    watchlistResponseSchema, // ✨ validate cấu trúc JSON; sai định dạng → tự fallback (giống portfolio-insights)
  );

  // ✨ Ép cứng tp/sl quanh currentPrice (giá trị server tự tính, KHÔNG phải do LLM trả) —
  // không tin LLM tự tuân thủ "stop_loss = entry × 0.93" trong prompt. Dùng currentPrice
  // làm mốc (không dùng `entry` do AI trả) vì đây là giá trị đáng tin duy nhất ở bước này.
  // ✨ Cũng clamp sniper_points.stop_loss / take_profit (field text) nếu có.
  const currentPriceMap = new Map(enrichedContext.map(item => [item.symbol, item.currentPrice]));
  const safePicks = aiCallResult.data.picks.map(p => {
    const currentPrice = currentPriceMap.get(p.symbol) ?? 0;
    let enforced = enforceTpSl(p, currentPrice);
    enforced = enforceAndClampSniperPoints(enforced, currentPrice);
    return enforced;
  });

  const finalResponse: WatchlistScanResponse = {
    ...aiCallResult.data,
    picks: safePicks, // ✨ dùng picks đã enforce tp/sl + sniper_points
