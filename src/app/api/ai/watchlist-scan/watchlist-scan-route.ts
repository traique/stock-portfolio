...
=== TRƯỜNG BẮT BUỘC TRONG PICKS (mỗi pick phải có đủ) ===
- trend_score: số từ 0-100 (lấy từ trendScore trong context, hoặc tự tính: MA alignment + RSI + MACD)
- bias_status: một trong nguy_hiem/canh_giac/an_toan/chiet_khau/qua_ban (lấy từ biasMA5 trong context)
- ma_alignment: bullish/bearish/mixed (lấy từ maAlignment trong context)
- sniper_points: PHẢI có số giá cụ thể (VND) kèm mô tả ngắn. Ví dụ: "13.700 (vùng hỗ trợ)" chứ không chỉ "Vùng hỗ trợ gần".
  Nếu không có dữ liệu support/resistance → dùng entry±3% cho ideal_buy, entry-7% cho stop_loss, tp cho take_profit.
...
Trả về DUY NHẤT JSON hợp lệ:
{
  ...
  "picks": [{
    ...
    "action_checklist": ["✅/⚠️/❌ điểm 1", "✅/⚠️/❌ điểm 2", "✅/⚠️/❌ điểm 3"],
    "sniper_points": { "ideal_buy": "13.700 (vùng hỗ trợ gần)", "secondary_buy": "13.200 (đáy tích lũy)", "stop_loss": "12.600 (dưới đáy tích lũy)", "take_profit": "15.540 (kháng cự mạnh)" },
    "trend_score": 72,
    "bias_status": "an_toan",
...
export async function POST(request: NextRequest) {
  ...
  // ✨ mozy lesson 3 — intraday mode: skip AI, return cached or fallback
