...
// Vercel serverless sẽ reuse instance trong cùng 1 deployment window,
// nên cache này hoạt động tốt cho dữ liệu ít thay đổi như model list.
...
// Filter: chỉ lấy model generateContent (bỏ embedding, vision-only, deprecated)
...
// ─── Fetch từ Groq ────────────────────────────────────────────────────────────
// API: GET https://api.groq.com/openai/v1/models
// Filter: chỉ lấy model active, bỏ whisper/vision
  ...
  try {
    ...
    return rawModels
      ...
      .map(m => {
        ...
        // Format label: "llama-3.3-70b-versatile" → "Llama 3.3 70B Versatile"
        const label = key
          .split('-')
...
// Giữ lại các model đã biết chắc hoạt động, tránh UI trống rỗng.
