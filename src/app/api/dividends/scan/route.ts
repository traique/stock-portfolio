...
// Validate body trước khi ghi DB (đồng bộ chuẩn zod của toàn codebase)
const postItemSchema = z.object({
  symbol: z.string().trim().regex(/^[A-Za-z0-9]{2,10}$/),
  kind: z.enum(['CASH', 'STOCK']),
  exDate: z.string().trim().regex(/^\d{4}-\d{2}-\d{2}$/),
  cashAmount: z.coerce.number().positive().optional(),
  stockShares: z.coerce.number().positive().optional(),
  note: z.string().trim().max(500).optional().nullable(),
});
const postBodySchema = z.object({
  items: z.array(postItemSchema).min(1).max(50),
});
  ...
  for (const it of items) {
    try {
      if (it.kind === 'STOCK') {
        ...
        // Loại 'DIVIDEND' đã được thêm vào check constraint (migration 0004).
