...
const QuoteCard = memo(function QuoteCard({ item, onRemove, onNews, flashKey }: {
  item: Quote;
  onRemove: (symbol: string) => void;
  onNews: (symbol: string) => void;
  /** Thay đổi mỗi lần giá làm mới → kích hoạt flash tăng/giảm */
  flashKey: number;
}) {
  ...
  // Flash khi giá vừa thay đổi (so giá hiện tại với giá của lần render trước)
  ...
  useEffect(() => {
    ...
    if (!prev || prev === item.price) return;
    setFlash(item.price > prev ? 'price-flash-up' : 'price-flash-down');
  ...
  return (
    <article style={QUOTE_ARTICLE_STYLE} className={flash}>
...
// ── Phiên giao dịch: pill trạng thái giúp user biết thời điểm theo dõi ──
// (thói quen quay lại trong giờ mở cửa = vòng lặp giữ chân tự nhiên)

type Session = { label: string; color: string; pulse: boolean };

function getVnSession(now: Date): Session {
  const dow  = now.getDay(); // 0=CN … 6=THỨ BẢY (giờ VN)
  const mins = now.getHours() * 60 + now.getMinutes();
  if (dow === 0 || dow === 6) return { label: 'CUỐI TUẦN', color: 'var(--muted)', pulse: false };
  if (mins >= 540 && mins < 690)  return { label: 'ĐANG MỞ CỬA', color: 'var(--green)', pulse: true };  // 9:00–11:30
  if (mins >= 690 && mins < 780)  return { label: 'NGHỈ TRƯA',  color: '#f59e0b',      pulse: false }; // 11:30–13:00
  if (mins >= 780 && mins < 900)  return { label: 'ĐANG MỞ CỬA', color: 'var(--green)', pulse: true };  // 13:00–15:00
  return { label: 'ĐÃ ĐÓNG', color: 'var(--muted)', pulse: false };
}
...
function MarketSessionPill() {
  ...
  return (
    <span
      role="status"
      aria-label={`Phiên giao dịch: ${session.label}`}
      style={ {
        display: 'inline-flex', alignItems: 'center', gap: 5,
        fontSize: 9, fontWeight: 900, letterSpacing: '0.07em',
        padding: '3px 9px', borderRadius: 99,
        background: 'var(--soft)', border: '1px solid var(--border)',
        color: session.color, whiteSpace: 'nowrap',
      } }
    >
      <span
        style={ {
          width: 6, height: 6, borderRadius: '50%', background: session.color,
          animation: session.pulse ? 'session-pulse 1.6s ease-in-out infinite' : 'none',
        } }
      />
      {session.label}
    </span>
...
const NewsModal = memo(function NewsModal({ modal, onClose }: { modal: NewsModalState; onClose: () => void }) {
  return (
    <Modal open={modal.isOpen} onClose={onClose} ariaLabel={`Tin tức ${modal.symbol}`}>
...
export default function HomePage() {
  ...
  const { quotes, vnIndex, loading, marketError, breadth, topPositive, updatedAt } =
        ...
        <div style={MAIN_GRID_STYLE}>
          <section className="ab-premium-card" style={CARD_STYLE}>
            ...
            {loading ? (
              ...
              <div className="ab-quote-grid" style={QUOTE_GRID_STYLE}>
                {quotes.map((item) => (
                  <QuoteCard key={item.symbol} item={item} onRemove={removeSymbol}
                    onNews={handleOpenNews} flashKey={updatedAt} />
                ))}
              </div>
            )}
          </section>
