  ...
  return <main className="ab-page"><div className="ab-shell premium-gap">
    ...
    <section className="ab-home-grid single-focus">
      <section className="ab-premium-card ab-watch-shell">
        ...
        <div className="ab-watch-grid premium">{quotes.map((item) => <article key={item.symbol} className="ab-watch-card premium"><div className="ab-row-between align-start"><div className="ab-symbol-wrap"><div className="ab-symbol premium">{item.symbol}</div></div><button type="button" className="ab-delete ghost" onClick={() => removeWatchSymbol(item.symbol)}>Xóa</button></div><div className="ab-price premium">{formatPrice(item.price)}</div><div className="ab-soft-change under-price" style={{ color: colorFor(item.change) }}>{formatPrice(item.change)} · {formatPct(item.pct)}</div></article>)}{!loading && quotes.length === 0 ? <div className="ab-note">Chưa có mã</div> : null}</div>
      </section>
