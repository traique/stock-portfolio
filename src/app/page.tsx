...
// ── ✨ Bias MA badge ─────────────────────────────────────────────────────────
function BiasBadge({ status, bias }: { status?: string; bias?: number }) {
  if (!status || bias === undefined) return null;
  const map: Record<string, { bg: string; color: string; label: string }> = {
    nguy_hiem: { bg: 'var(--red-surface)',   color: 'var(--red)',   label: 'MUA ĐUỔI ⚠' },
    canh_giac: { bg: 'rgba(245,158,11,0.12)',color: 'var(--yellow)',label: 'CẨN THẬN'    },
    an_toan:   { bg: 'var(--soft)',           color: 'var(--muted)', label: 'AN TOÀN'     },
...
function TrendScoreBar({ score }: { score?: number }) {
  ...
  return (
    <div title={`Trend Score: ${score}/100`} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
      ...
      <span className="num-premium" style={{ fontSize: 10, fontWeight: 800, color, minWidth: 24, textAlign: 'right' }}>
        {score}
      </span>
    </div>
  );
...
function SniperPoints({ points }: { points?: AiPick['sniper_points'] }) {
  ...
  const rows = [
    { label: 'Điểm lý tưởng', value: points.ideal_buy,     color: 'var(--green)' },
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4px 12px' }}>
        {rows.map(r => (
          <div key={r.label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontSize: 10, color: 'var(--muted)' }}>{r.label}</span>
            <span className="num-premium" style={{ fontSize: 11, fontWeight: 800, color: r.color }}>{r.value}</span>
          </div>
        ))}
      </div>
    </div>
      ...
      <div style={{ padding: '12px 14px 8px' }}>
        <div className="ab-row-between align-center" style={{ marginBottom: 6 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <BiasBadge status={pick.bias_status} bias={undefined} />
        ...
        {/* Trend score bar */}
        {pick.trend_score !== undefined && <TrendScoreBar score={pick.trend_score} />}

        {/* Time sensitivity */}
        {pick.time_sensitivity && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginTop: 6 }}>
            <Clock size={10} color="var(--muted)" />
            <span style={{ fontSize: 11, color: 'var(--muted)', fontWeight: 600 }}>{pick.time_sensitivity}</span>
          </div>
        )}
