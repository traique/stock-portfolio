...
import { ArrowDownRight, ArrowUpRight, ChevronDown, ChevronUp, Landmark, PieChart, Send, TrendingDown, TrendingUp, Wallet } from 'lucide-react';
import { useCallback, useEffect, useMemo, useState } from 'react';
import AppShellHeader from '@/components/app-shell-header';
import { calcCashSummary, calcPosition, calcRealizedSummary, calcSummary, CashTransaction, deriveOpenHoldings, enrichTransactions, formatCurrency, groupHoldingsBySymbol, PortfolioSettings, PriceMap, Transaction } from '@/lib/calculations';
...
export default function DashboardPage() {
  ...
  const premiumCardStyle = { borderRadius: 28, background: 'linear-gradient(180deg, rgba(255,255,255,0.98), rgba(247,249,252,0.96))', boxShadow: '0 20px 50px rgba(15,23,42,0.06), inset 0 1px 0 rgba(255,255,255,0.72)', border: '1px solid rgba(148,163,184,0.12)' } as const;
  const premiumButtonStyle = { borderRadius: 18, boxShadow: '0 12px 28px rgba(15,23,42,0.10)' } as const;
  const premiumInputStyle = { borderRadius: 22, background: 'rgba(255,255,255,0.92)', boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.75)' } as const;
  ...
  const historyRows = useMemo(() => {
    ...
    return [...tradeRows, ...cashRows]
      .filter((row) => {
        ...
        return row.kind === 'trade' ? row.item.transaction_type === historyFilter : row.item.transaction_type === historyFilter;
      })
      .filter((row) => {
    ...
    <main className="ab-page">
      <div className="ab-shell premium-gap">
        ...
        <section className="ab-summary-grid premium-summary-grid compact-top-grid">
          {loading ? (
            <>
              <SummarySkeleton /><SummarySkeleton /><SummarySkeleton /><SummarySkeleton />
            </>
          ) : (
            <>
              <article className="ab-premium-card ab-stat-premium neutral" style={premiumCardStyle}>
                <div className="ab-stat-head"><Landmark size={16} /><span className="ab-soft-label">Tổng vốn</span></div>
                <div className="ab-big-number dark">{formatCurrency(totalCapital)}</div>
              </article>
              <article className="ab-premium-card ab-stat-premium neutral" style={premiumCardStyle}>
                <div className="ab-stat-head"><Wallet size={16} /><span className="ab-soft-label">NAV thực tế</span></div>
                <div className="ab-big-number dark">{formatCurrency(actualNav)}</div>
              </article>
              <article className="ab-premium-card ab-stat-premium neutral" style={premiumCardStyle}>
                <div className="ab-stat-head"><PieChart size={16} /><span className="ab-soft-label">Giá trị thị trường</span></div>
                <div className="ab-big-number dark">{formatCurrency(marketValue)}</div>
              </article>
              <article className="ab-premium-card ab-stat-premium neutral" style={premiumCardStyle}>
                <div className="ab-stat-head"><TrendingUp size={16} /><span className="ab-soft-label">Tổng tài sản</span></div>
        ...
        {!loading ? (
          <section className="ab-summary-grid premium-summary-grid compact-top-grid">
            <article className={`ab-premium-card ab-stat-premium ${statTone(totalPnl)}`} style={premiumCardStyle}>
              <div className="ab-stat-head"><TrendingUp size={16} /><span className="ab-soft-label">Tổng lãi/lỗ</span></div>
              <div className="ab-big-number" style={{ color: getChangeColor(totalPnl) }}>{formatCurrency(totalPnl)}</div>
              <div className="ab-stat-sub" style={{ color: getChangeColor(totalPnl) }}>{totalPnlPct >= 0 ? '+' : ''}{totalPnlPct.toFixed(2)}%</div>
            </article>
            <article className={`ab-premium-card ab-stat-premium ${statTone(dayPnl)}`} style={premiumCardStyle}>
              <div className="ab-stat-head">{dayPnl >= 0 ? <ArrowUpRight size={16} /> : <ArrowDownRight size={16} />}<span className="ab-soft-label">Lãi/lỗ trong ngày</span></div>
              <div className="ab-big-number" style={{ color: getChangeColor(dayPnl) }}>{formatCurrency(dayPnl)}</div>
              <div className="ab-stat-sub">{refreshing ? 'Đang cập nhật giá...' : 'Theo biến động phiên hiện tại'}</div>
            </article>
            <article className={`ab-premium-card ab-stat-premium ${statTone(unrealizedPnl)}`} style={premiumCardStyle}>
              <div className="ab-stat-head"><TrendingUp size={16} /><span className="ab-soft-label">Lãi/lỗ cổ phiếu đang giữ</span></div>
              <div className="ab-big-number" style={{ color: getChangeColor(unrealizedPnl) }}>{formatCurrency(unrealizedPnl)}</div>
              <div className="ab-stat-sub">Hiệu suất vị thế mở</div>
            </article>
            <article className={`ab-premium-card ab-stat-premium ${statTone(realizedSummary.totalRealizedPnl)}`} style={premiumCardStyle}>
              <div className="ab-stat-head"><TrendingDown size={16} /><span className="ab-soft-label">Lãi/lỗ đã chốt</span></div>
              <div className="ab-big-number" style={{ color: getChangeColor(realizedSummary.totalRealizedPnl) }}>{formatCurrency(realizedSummary.totalRealizedPnl)}</div>
              <div className="ab-stat-sub" style={{ color: getChangeColor(realizedSummary.totalRealizedPnl) }}>{realizedSummary.totalSellOrders} lệnh bán</div>
            </article>
          </section>
        ) : null}
        ...
        {vnIndex ? (
          <section className="ab-premium-card ab-form-shell compact" style={premiumCardStyle}>
            <div className="ab-row-between align-center">
              ...
              <div className="ab-soft-change under-price" style={{ color: getChangeColor(vnIndex.change), padding: '10px 14px', borderRadius: 999, background: 'rgba(255,255,255,0.72)' }}>
              ...
              {allocations.map((item) => (
                <div key={item.symbol} style={{ display: 'grid', gap: 8 }}>
                  <div className="ab-row-between align-center">
                    ...
                    <div className="ab-mini-price">{formatCurrency(item.totalNow)} · {item.percent.toFixed(1)}%</div>
                  </div>
                  <div style={{ width: '100%', height: 10, borderRadius: 999, background: 'rgba(148,163,184,0.16)', overflow: 'hidden' }}>
                    <div style={{ width: `${Math.max(item.percent, 2)}%`, height: '100%', borderRadius: 999, background: 'linear-gradient(90deg, rgba(37,99,235,0.95), rgba(96,165,250,0.65))' }} />
                  </div>
                </div>
              ))}
            </div>
        ...
        {message ? (
          <section className="ab-premium-card ab-form-shell compact" style={premiumCardStyle}><div className="ab-error">{message}</div></section>
        ) : null}

        {loading ? (
          <section className="ab-position-grid">
            <article className="ab-premium-card ab-position-card ab-skeleton-card" style={premiumCardStyle}><div className="ab-skeleton skeleton-title" /><div className="ab-skeleton skeleton-price" /><div className="ab-skeleton skeleton-line" /><div className="ab-skeleton skeleton-line short" /></article>
            <article className="ab-premium-card ab-position-card ab-skeleton-card" style={premiumCardStyle}><div className="ab-skeleton skeleton-title" /><div className="ab-skeleton skeleton-price" /><div className="ab-skeleton skeleton-line" /><div className="ab-skeleton skeleton-line short" /></article>
          </section>
        ) : positions.length === 0 ? (
          <section className="ab-premium-card ab-form-shell compact" style={premiumCardStyle}><div className="ab-note">Chưa có vị thế mở nào trong danh mục</div></section>
        ) : (
          <section className="ab-position-grid">
            {positions.map((position) => {
              ...
              return (
                <article key={position.symbol} className="ab-premium-card ab-position-card" style={premiumCardStyle}>
                  <div className="ab-row-between align-start">
                    <div>
                      <div className="ab-symbol premium">{position.symbol}</div>
                      <div className="ab-soft-label mini-top">{position.holdings.length} lệnh mua mở · SL {position.quantity}</div>
                    </div>
                    <button type="button" className="ab-delete ghost" onClick={() => toggleSymbol(position.symbol)} style={{ borderRadius: 999, padding: '8px 12px', background: 'rgba(248,250,252,0.9)' }}>
                  ...
                  <div className="ab-position-stats">
                    <div className="ab-stat-chip"><span>SL tổng</span><strong>{position.quantity}</strong></div>
                    <div className="ab-stat-chip"><span>Giá vốn TB</span><strong>{formatCurrency(position.avgBuyPrice)}</strong></div>
                  </div>
                  ...
                  {isExpanded ? (
                    <div className="ab-mini-list" style={{ marginTop: 14 }}>
                      {position.holdings.map((holding) => (
                        <div key={holding.id} className="ab-mini-row"><div><div className="ab-mini-symbol">{formatTradeDate(holding.buy_date)} · SL {holding.quantity}</div><div className="ab-mini-price">Giá mua {formatCurrency(Number(holding.buy_price))}</div></div></div>
                      ))}
                    </div>
                  ) : null}
                </article>
        ...
        <section className="ab-premium-card ab-form-shell compact" style={premiumCardStyle}>
          <button type="button" className="ab-section-toggle" onClick={() => setBuyOpen((v) => !v)}>
            <div className="ab-section-toggle-copy"><div className="ab-card-kicker">Giao dịch</div><div className="ab-section-toggle-title">{editingTradeId && buyOpen ? 'Sửa lệnh mua' : 'Thêm lệnh mua'}</div></div>
            <div className="ab-section-toggle-icon">{buyOpen ? <ChevronUp size={18} /> : <ChevronDown size={18} />}</div>
          </button>
          {buyOpen ? (
            <form onSubmit={handleBuySubmit} className="ab-form-grid compact-form-grid mt-16">
              ...
              <div className="ab-row-gap">
                <button type="submit" className="ab-btn ab-btn-primary" style={premiumButtonStyle}>{editingTradeId ? 'Lưu lệnh mua' : 'Thêm lệnh mua'}</button>
                {editingTradeId ? <button type="button" className="ab-btn ab-btn-subtle" onClick={() => { setEditingTradeId(null); setBuyForm(DEFAULT_BUY_FORM); setBuyOpen(false); }} style={premiumButtonStyle}>Hủy</button> : null}
              </div>
            </form>
          ) : null}
        </section>
        ...
        <section className="ab-premium-card ab-form-shell compact" style={premiumCardStyle}>
          <button type="button" className="ab-section-toggle" onClick={() => setSellOpen((v) => !v)}>
            <div className="ab-section-toggle-copy"><div className="ab-card-kicker">Giao dịch</div><div className="ab-section-toggle-title">{editingTradeId && sellOpen ? 'Sửa lệnh bán' : 'Thêm lệnh bán'}</div></div>
            <div className="ab-section-toggle-icon">{sellOpen ? <ChevronUp size={18} /> : <ChevronDown size={18} />}</div>
          </button>
          {sellOpen ? (
            <form onSubmit={handleSellSubmit} className="ab-form-grid compact-form-grid mt-16">
              ...
              <div className="ab-row-gap">
                <button type="submit" className="ab-btn ab-btn-primary" style={premiumButtonStyle}>{editingTradeId ? 'Lưu lệnh bán' : 'Thêm lệnh bán'}</button>
                {editingTradeId ? <button type="button" className="ab-btn ab-btn-subtle" onClick={() => { setEditingTradeId(null); setSellForm(DEFAULT_SELL_FORM); setSellOpen(false); }} style={premiumButtonStyle}>Hủy</button> : null}
              </div>
            </form>
          ) : null}
        </section>
        ...
        <section className="ab-premium-card ab-form-shell compact" style={premiumCardStyle}>
          <button type="button" className="ab-section-toggle" onClick={() => setHistoryOpen((v) => !v)}>
            <div className="ab-section-toggle-copy"><div className="ab-card-kicker">Giao dịch</div><div className="ab-section-toggle-title">Nhật ký giao dịch</div></div>
            <div className="ab-section-toggle-icon">{historyOpen ? <ChevronUp size={18} /> : <ChevronDown size={18} />}</div>
          </button>
          {historyOpen ? (
            <>
              <div className="ab-row-gap mt-16">
                <select value={historyFilter} onChange={(e) => setHistoryFilter(e.target.value as TxTypeFilter)} className="ab-input" style={premiumInputStyle}>
                ...
                <input value={historySymbol} onChange={(e) => setHistorySymbol(e.target.value)} placeholder="Lọc theo mã" className="ab-input" style={premiumInputStyle} />
              </div>
              <div className="ab-mini-list mt-16">
                {historyRows.length ? historyRows.map((row) => {
                  if (row.kind === 'trade') {
                    ...
                    return <div key={item.id} className="ab-mini-row"><div><div className="ab-mini-symbol">{getTransactionLabel(item.transaction_type)} · {item.symbol} · SL {item.quantity}</div><div className="ab-mini-price">{formatTradeDate(item.trade_date)} · Giá {formatCurrency(Number(item.price))}{realizedText}</div></div><div className="ab-row-gap"><button type="button" className="ab-btn ab-btn-subtle" onClick={() => editTrade(item)} style={premiumButtonStyle}>Sửa</button><button type="button" className="ab-delete ghost" onClick={() => deleteTrade(item)}>Xóa</button></div></div>;
                  }
                  const item = row.item;
                  return <div key={item.id} className="ab-mini-row"><div><div className="ab-mini-symbol">{getTransactionLabel(item.transaction_type)}</div><div className="ab-mini-price">{formatTradeDate(item.transaction_date)} · {formatCurrency(Number(item.amount))}</div></div><div className="ab-row-gap"><button type="button" className="ab-btn ab-btn-subtle" onClick={() => editCash(item)} style={premiumButtonStyle}>Sửa</button><button type="button" className="ab-delete ghost" onClick={() => deleteCash(item)}>Xóa</button></div></div>;
        ...
        <section className="ab-premium-card ab-form-shell compact" style={premiumCardStyle}>
          <button type="button" className="ab-section-toggle" onClick={() => setCashOpen((v) => !v)}>
            <div className="ab-section-toggle-copy"><div className="ab-card-kicker">Tiền mặt</div><div className="ab-section-toggle-title">Nạp / Rút / Điều chỉnh tiền mặt</div></div>
            <div className="ab-section-toggle-icon">{cashOpen ? <ChevronUp size={18} /> : <ChevronDown size={18} />}</div>
          </button>
          {cashOpen ? (
            <>
              <div className="ab-row-gap mt-16">
                <button type="button" className={`ab-btn ${cashMode === 'CASH' ? 'ab-btn-primary' : 'ab-btn-subtle'}`} onClick={() => setCashMode('CASH')} style={premiumButtonStyle}>Nạp / Rút tiền</button>
                <button type="button" className={`ab-btn ${cashMode === 'ADJUSTMENT' ? 'ab-btn-primary' : 'ab-btn-subtle'}`} onClick={() => setCashMode('ADJUSTMENT')} style={premiumButtonStyle}>Điều chỉnh tiền mặt</button>
              </div>
              {cashMode === 'CASH' ? (
                <form onSubmit={handleCashSubmit} className="ab-form-grid compact-form-grid mt-16">
                  <select value={cashForm.transaction_type} onChange={(e) => setCashForm({ ...cashForm, transaction_type: e.target.value as 'DEPOSIT' | 'WITHDRAW' })} className="ab-input" style={premiumInputStyle}><option value="DEPOSIT">Nạp tiền</option><option value="WITHDRAW">Rút tiền</option></select>
                  <input value={cashForm.amount} onChange={(e) => setCashForm({ ...cashForm, amount: e.target.value })} type="number" placeholder="Số tiền" required className="ab-input" style={premiumInputStyle} />
                  <input value={cashForm.transaction_date} onChange={(e) => setCashForm({ ...cashForm, transaction_date: e.target.value })} type="date" className="ab-input" style={premiumInputStyle} />
                  <input value={cashForm.note} onChange={(e) => setCashForm({ ...cashForm, note: e.target.value })} placeholder="Ghi chú" className="ab-input ab-full" style={premiumInputStyle} />
                  <div className="ab-row-gap">
                    <button type="submit" className="ab-btn ab-btn-primary" style={premiumButtonStyle}>{editingCashId ? 'Lưu giao dịch tiền' : 'Lưu giao dịch tiền'}</button>
                    {editingCashId ? <button type="button" className="ab-btn ab-btn-subtle" onClick={() => { setEditingCashId(null); setCashForm(DEFAULT_CASH_FORM); }} style={premiumButtonStyle}>Hủy</button> : null}
                  </div>
                </form>
              ) : (
                <form onSubmit={handleSaveCashAdjustment} className="ab-form-grid compact-form-grid mt-16">
                  <div className="ab-row-gap">
                    <button type="button" className={`ab-btn ${adjustmentSign === 1 ? 'ab-btn-primary' : 'ab-btn-subtle'}`} onClick={() => setAdjustmentSign(1)} style={premiumButtonStyle}>Dương (+)</button>
                    <button type="button" className={`ab-btn ${adjustmentSign === -1 ? 'ab-btn-primary' : 'ab-btn-subtle'}`} onClick={() => setAdjustmentSign(-1)} style={premiumButtonStyle}>Âm (-)</button>
                  </div>
                  <input value={adjustmentAmountInput} onChange={(e) => setAdjustmentAmountInput(e.target.value)} type="number" inputMode="decimal" className="ab-input" placeholder="Nhập số điều chỉnh" style={premiumInputStyle} />
                  <div className="ab-note">Tiền mặt tính toán: <strong>{formatCurrency(cashSummary.calculatedCash)}</strong></div>
                  <div className="ab-note">Điều chỉnh hiện tại: <strong>{cashSummary.cashAdjustment >= 0 ? '+' : ''}{formatCurrency(cashSummary.cashAdjustment)}</strong></div>
        ...
        <section className="ab-premium-card ab-form-shell compact" style={premiumCardStyle}>
          <button type="button" className="ab-section-toggle" onClick={() => setTelegramOpen((v) => !v)}>
            <div className="ab-section-toggle-copy"><div className="ab-card-kicker">Telegram</div><div className="ab-section-toggle-title">Báo cáo cuối ngày</div></div>
            <div className="ab-section-toggle-icon">{telegramOpen ? <ChevronUp size={18} /> : <ChevronDown size={18} />}</div>
          </button>
          {telegramOpen ? (
            <>
              <form onSubmit={handleSaveTelegram} className="ab-form-grid compact-form-grid mt-16">
                <input value={telegram.chat_id} onChange={(e) => setTelegram({ ...telegram, chat_id: e.target.value })} placeholder="Nhập chat_id Telegram" className="ab-input ab-full" style={premiumInputStyle} />
                <label className="ab-toggle-row"><input type="checkbox" checked={telegram.is_enabled} onChange={(e) => setTelegram({ ...telegram, is_enabled: e.target.checked })} /><span>Bật báo cáo Telegram</span></label>
                <label className="ab-toggle-row"><input type="checkbox" checked={telegram.notify_daily} onChange={(e) => setTelegram({ ...telegram, notify_daily: e.target.checked })} /><span>Nhận báo cáo cuối ngày</span></label>
                <input value={telegram.daily_hour_vn} onChange={(e) => setTelegram({ ...telegram, daily_hour_vn: clampHour(Number(e.target.value || 15)) })} type="number" min={0} max={23} className="ab-input" placeholder="Giờ Việt Nam" style={premiumInputStyle} />
                ...
                <div className="ab-row-gap">
                  ...
                  <button type="button" className="ab-btn ab-btn-subtle" onClick={handleTelegramTest} disabled={telegramTesting || telegramLoading} style={premiumButtonStyle}><Send size={14} />{telegramTesting ? 'Đang gửi...' : 'Gửi báo cáo'}</button>
                </div>
              </form>
