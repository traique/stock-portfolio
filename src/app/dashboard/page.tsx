...
export default function DashboardPage() {
  ...
  async function handleTradeSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (editingTradeId) {
      const { error } = await supabase.from('transactions').update({
        ...
        avg_cost: null,
        realized_pnl: null,
      }).eq('id', editingTradeId).eq('user_id', userId);
  ...
  async function handleCashSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (editingCashId) {
      ...
      }).eq('id', editingCashId).eq('user_id', userId);
      if (error) { setMessage(error.message); return; }
    } else {
  ...
  return (
    <main className="ab-page">
      <div className="ab-shell premium-gap" style={{ gap: 12 }}>
        ...
        {!loading && positions.length ? <section className="ab-position-grid" style={{ gap: 12 }}>{positions.map((position) => {
          const row = calcPosition(position, prices);
          ...
          return <article key={position.symbol} style={{ ...cardStyle, padding: 14 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginTop: 10 }}><div style={{ ...strongCardStyle, padding: 10, borderRadius: 16, boxShadow: 'none' }}><div style={{ fontSize: 11, color: muted() }}>Tổng mua</div><div style={{ fontSize: 14, fontWeight: 800, marginTop: 4, color: fg() }}>{formatCurrency(row.totalBuy)}</div></div><div style={{ ...strongCardStyle, padding: 10, borderRadius: 16, boxShadow: 'none' }}><div style={{ fontSize: 11, color: muted() }}>Hiện tại</div><div style={{ fontSize: 14, fontWeight: 800, marginTop: 4, color: fg() }}>{formatCurrency(row.totalNow)}</div></div></div>
