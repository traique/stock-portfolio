  ...
  return (
    <main className="ab-page">
      <div className="ab-shell premium-gap" style={{ gap: 12 }}>
        ...
        <Section
          kicker="AI Assistant"
          title="Nhận xét danh mục + TP/SL thông minh"
          open={aiOpen}
          onToggle={() => setAiOpen((v) => !v)}
        >
          <div className="ab-row-gap">
            <button type="button" className="ab-btn ab-btn-primary" onClick={handleAiPortfolioInsights} disabled={aiLoading}>
              {aiLoading ? 'Đang phân tích...' : 'Phân tích danh mục'}
            </button>
          </div>
          {aiError ? <div className="ab-error" style={{ marginTop: 10 }}>{aiError}</div> : null}
          {aiResult?.cached ? (
            <div className="ab-note" style={{ marginTop: 8 }}>
              Kết quả cache ~{Math.round((aiResult.cache_ttl_seconds || 0) / 60)} phút để tránh spam API.
            </div>
          ) : null}
          {aiResult ? (
            <div style={{ marginTop: 12, display: 'grid', gap: 10 }}>
              <div className="ab-note">{aiResult.summary}</div>
              {(aiResult.actions || []).slice(0, 8).map((item) => (
                <div key={item.symbol} style={{ ...cardStyle, padding: 10, borderRadius: 14, boxShadow: 'none' }}>
                  <div className="ab-row-between align-center" style={{ marginBottom: 4 }}>
                    <strong>{item.symbol} · {item.action}</strong>
                    <span style={{ ...pillStyle }}>{item.confidence}</span>
                  </div>
                  <div className="ab-note">{item.reason}</div>
                  <div className="ab-note" style={{ marginTop: 4 }}>
                    TP: <strong>{Number.isFinite(Number(item.tp)) ? formatCurrency(Number(item.tp)) : '--'}</strong> ·
                    SL: <strong>{Number.isFinite(Number(item.sl)) ? formatCurrency(Number(item.sl)) : '--'}</strong>
                  </div>
                </div>
              ))}
              {aiResult.risks?.length ? <div className="ab-note">Rủi ro: {aiResult.risks.join(' | ')}</div> : null}
            </div>
          ) : <div className="ab-note" style={{ marginTop: 8 }}>Nhấn “Phân tích danh mục” để AI đưa gợi ý xử lý và TP/SL.</div>}
        </Section>
