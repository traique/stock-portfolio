...
// =========================================================
// SHARED TYPES  (imported by child components via this file)
// =========================================================
...
const AI_CACHE_KEY = 'lcta_ai_portfolio_result';
...
export default function DashboardPage() {
  ...
  const loadPortfolio = useCallback(async (uid?: string, em?: string) => {
    ...
    if (txRes.error)       { setTransactions([]);    setMessage(txRes.error.message); }
    else                    setTransactions((txRes.data ?? []) as Transaction[]);

    if (cashRes.error)     { setCashTransactions([]); if (!txRes.error) setMessage(cashRes.error.message); }
  ...
  const allocations = useMemo<AllocationItem[]>(() => {
    ...
    return positions
      .map(pos => {
        ...
        return { symbol: pos.symbol, totalNow: row.totalNow, percent: total > 0 ? (row.totalNow / total) * 100 : 0 };
      })
  ...
  return (
    <main className="ab-page">
      <div className="ab-shell" style={{ gap: 12 }}>
        ...
        {/* READ — overview, allocation, holdings */}
        <PortfolioView
          loading={loading}
        ...
        {/* WRITE — forms, history, AI, telegram, reset */}
        <DashboardActions
          userId={userId}
          email={email}
