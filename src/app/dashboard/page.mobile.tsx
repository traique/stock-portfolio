...
const chipStyle = {
  borderRadius: 999,
  padding: '6px 10px',
  background: 'var(--secondary, rgba(148,163,184,0.14))',
  border: '1px solid var(--border, rgba(148,163,184,0.18))',
  color: 'var(--foreground, inherit)',
  fontSize: 12,
  fontWeight: 700,
...
const inputStyle = {
  borderRadius: 18,
  background: 'var(--input, rgba(255,255,255,0.96))',
  color: 'var(--foreground, inherit)',
  border: '1px solid var(--border, rgba(148,163,184,0.18))',
...
function getChangeColor(value?: number | null) {
  if (value === null || value === undefined || !Number.isFinite(value)) return 'var(--muted, #64748b)';
  if (value > 0) return 'var(--green, #16a34a)';
  if (value < 0) return 'var(--red, #dc2626)';
...
function Section({ title, kicker, open, onToggle, children }: { title: string; kicker: string; open: boolean; onToggle: () => void; children: React.ReactNode }) {
  return (
    <section className="ab-premium-card" style={{ ...shellCardStyle, padding: 14 }}>
      <button type="button" className="ab-section-toggle" onClick={onToggle} style={{ minHeight: 'unset' }}>
        <div className="ab-section-toggle-copy">
          <div className="ab-card-kicker">{kicker}</div>
          <div className="ab-section-toggle-title" style={{ fontSize: 18 }}>{title}</div>
        </div>
        <div className="ab-section-toggle-icon">{open ? <ChevronUp size={18} /> : <ChevronDown size={18} />}</div>
      </button>
...
function StatCard({ label, value, subValue, icon, tone = 'neutral' }: { label: string; value: string; subValue?: string; icon: React.ReactNode; tone?: 'neutral' | 'up' | 'down' }) {
  const color = tone === 'up' ? 'var(--green, #16a34a)' : tone === 'down' ? 'var(--red, #dc2626)' : 'var(--foreground, #0f172a)';
  return (
    <article className="ab-premium-card" style={{ ...shellCardStyle, padding: 14 }}>
      <div className="ab-stat-head" style={{ color: tone === 'neutral' ? 'var(--muted, #64748b)' : color, marginBottom: 6 }}>
        {icon}
        <span className="ab-soft-label">{label}</span>
      </div>
      <div style={{ fontSize: 18, fontWeight: 900, lineHeight: 1.15, color }}>{value}</div>
      {subValue ? <div style={{ fontSize: 12, marginTop: 4, color }}>{subValue}</div> : null}
    </article>
  );
}
    ...
    <main className="ab-page">
      <div className="ab-shell premium-gap" style={{ gap: 12 }}>
        ...
        <section className="ab-summary-grid premium-summary-grid compact-top-grid" style={{ gap: 10 }}>
          {loading ? <><StatCard label="Tổng vốn" value="..." icon={<Landmark size={16} />} /><StatCard label="NAV thực tế" value="..." icon={<Wallet size={16} />} /><StatCard label="Giá trị thị trường" value="..." icon={<PieChart size={16} />} /><StatCard label="Tổng tài sản" value="..." icon={<TrendingUp size={16} />} /></> : <>
            <StatCard label="Tổng vốn" value={formatCurrency(totalCapital)} icon={<Landmark size={16} />} />
            <StatCard label="NAV thực tế" value={formatCurrency(actualNav)} icon={<Wallet size={16} />} />
            <StatCard label="Giá trị thị trường" value={formatCurrency(marketValue)} icon={<PieChart size={16} />} />
            <StatCard label="Tổng tài sản" value={formatCurrency(totalAssets)} icon={<TrendingUp size={16} />} />
          </>}
        </section>
        ...
        {vnIndex ? <section className="ab-premium-card" style={{ ...shellCardStyle, padding: 14 }}><div className="ab-row-between align-center"><div><div className="ab-card-kicker">VN-Index</div><div style={{ fontWeight: 900, fontSize: 18 }}>{formatCompactPrice(vnIndex.price)}</div></div><div style={{ ...chipStyle, color: getChangeColor(vnIndex.change) }}>{formatChange(vnIndex.change)} · {formatPct(vnIndex.pct)}</div></div></section> : null}

        {!!allocations.length ? <section className="ab-premium-card" style={{ ...shellCardStyle, padding: 14 }}><div className="ab-card-kicker" style={{ marginBottom: 10 }}>Cơ cấu danh mục</div><div style={{ display: 'grid', gap: 10 }}>{allocations.map((item) => <div key={item.symbol} style={{ display: 'grid', gap: 6 }}><div className="ab-row-between align-center"><div style={{ fontWeight: 800 }}>{item.symbol}</div><div style={{ fontSize: 12, color: 'var(--muted, #64748b)' }}>{formatCurrency(item.totalNow)} · {item.percent.toFixed(1)}%</div></div><div style={{ width: '100%', height: 8, borderRadius: 999, background: 'var(--secondary, rgba(148,163,184,0.14))', overflow: 'hidden' }}><div style={{ width: `${Math.max(item.percent, 2)}%`, height: '100%', borderRadius: 999, background: 'linear-gradient(90deg, rgba(37,99,235,0.95), rgba(96,165,250,0.65))' }} /></div></div>)}</div></section> : null}

        {message ? <section className="ab-premium-card" style={{ ...shellCardStyle, padding: 14 }}><div className="ab-error">{message}</div></section> : null}

        {loading ? <section className="ab-position-grid"><div style={{ ...shellCardStyle, padding: 14 }}>Đang tải...</div><div style={{ ...shellCardStyle, padding: 14 }}>Đang tải...</div></section> : !positions.length ? <section className="ab-premium-card" style={{ ...shellCardStyle, padding: 14 }}><div className="ab-note">Chưa có vị thế mở nào trong danh mục</div></section> : <section className="ab-position-grid" style={{ gap: 12 }}>{positions.map((position) => {
          ...
          return <article key={position.symbol} className="ab-premium-card" style={{ ...shellCardStyle, padding: 14 }}>
            <div className="ab-row-between align-start" style={{ marginBottom: 6 }}><div><div style={{ fontSize: 28, fontWeight: 900, lineHeight: 1 }}>{position.symbol}</div><div style={{ fontSize: 12, color: 'var(--muted, #64748b)', marginTop: 4 }}>{position.holdings.length} lệnh mua mở · SL {position.quantity}</div></div><button type="button" className="ab-delete ghost" onClick={() => setExpandedSymbols((prev) => ({ ...prev, [position.symbol]: !prev[position.symbol] }))} style={{ borderRadius: 999, padding: '6px 10px', background: 'var(--secondary, rgba(148,163,184,0.14))' }}>{isExpanded ? 'Ẩn lệnh' : 'Xem lệnh'}</button></div>
            <div style={{ fontSize: 22, fontWeight: 900 }}>{formatCompactPrice(quote?.price ?? row.currentPrice)}</div>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 10 }}><div style={chipStyle}>SL tổng {position.quantity}</div><div style={chipStyle}>Giá vốn TB {formatCurrency(position.avgBuyPrice)}</div></div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginTop: 10 }}><div style={{ ...shellCardStyle, padding: 10, borderRadius: 16, boxShadow: 'none' }}><div style={{ fontSize: 11, color: 'var(--muted, #64748b)' }}>Tổng mua</div><div style={{ fontSize: 14, fontWeight: 800, marginTop: 4 }}>{formatCurrency(row.totalBuy)}</div></div><div style={{ ...shellCardStyle, padding: 10, borderRadius: 16, boxShadow: 'none' }}><div style={{ fontSize: 11, color: 'var(--muted, #64748b)' }}>Hiện tại</div><div style={{ fontSize: 14, fontWeight: 800, marginTop: 4 }}>{formatCurrency(row.totalNow)}</div></div></div>
            <div style={{ display: 'grid', gap: 8, marginTop: 10 }}><div style={{ borderRadius: 14, padding: '10px 12px', background: positive ? 'rgba(22,163,74,0.10)' : 'rgba(220,38,38,0.10)', border: `1px solid ${positive ? 'rgba(22,163,74,0.22)' : 'rgba(220,38,38,0.22)'}`, color: positive ? 'var(--green, #16a34a)' : 'var(--red, #dc2626)', display: 'flex', justifyContent: 'space-between', gap: 8 }}><span style={{ fontWeight: 700 }}>Lãi / Lỗ</span><strong>{formatCurrency(row.pnl)}</strong></div><div style={{ borderRadius: 14, padding: '10px 12px', background: positive ? 'rgba(22,163,74,0.08)' : 'rgba(220,38,38,0.08)', border: `1px solid ${positive ? 'rgba(22,163,74,0.18)' : 'rgba(220,38,38,0.18)'}`, color: positive ? 'var(--green, #16a34a)' : 'var(--red, #dc2626)', display: 'flex', justifyContent: 'space-between', gap: 8 }}><span style={{ fontWeight: 700 }}>Hiệu suất vị thế</span><strong>{row.pnlPct >= 0 ? '+' : ''}{row.pnlPct.toFixed(2)}%</strong></div></div>
            {isExpanded ? <div style={{ display: 'grid', gap: 8, marginTop: 10 }}>{position.holdings.map((holding) => <div key={holding.id} style={{ ...shellCardStyle, padding: 10, borderRadius: 14, boxShadow: 'none' }}><div style={{ fontSize: 12, fontWeight: 700 }}>{formatTradeDate(holding.buy_date)} · SL {holding.quantity}</div><div style={{ fontSize: 12, color: 'var(--muted, #64748b)', marginTop: 3 }}>Giá mua {formatCurrency(Number(holding.buy_price))}</div></div>)}</div> : null}
          </article>;
        })}</section>}
        ...
        <Section kicker="Giao dịch" title="Nhật ký giao dịch" open={historyOpen} onToggle={() => setHistoryOpen((v) => !v)}>
          ...
          <div style={{ display: 'grid', gap: 8 }}>{historyRows.length ? historyRows.map((row) => row.kind === 'trade' ? <div key={row.item.id} style={{ ...shellCardStyle, padding: 10, borderRadius: 14, boxShadow: 'none' }}><div style={{ fontSize: 12, fontWeight: 800 }}>{getTransactionLabel(row.item.transaction_type)} · {row.item.symbol} · SL {row.item.quantity}</div><div style={{ fontSize: 12, color: 'var(--muted, #64748b)', marginTop: 4 }}>{formatTradeDate(row.item.trade_date)} · Giá {formatCurrency(Number(row.item.price))}{row.item.transaction_type === 'SELL' ? ` · Đã chốt ${formatCurrency(Number(row.item.realized_pnl || 0))}` : ''}</div><div className="ab-row-gap" style={{ marginTop: 8 }}><button type="button" className="ab-btn ab-btn-subtle" onClick={() => editTrade(row.item)} style={primaryBtnStyle}>Sửa</button><button type="button" className="ab-delete ghost" onClick={() => deleteTrade(row.item)}>Xóa</button></div></div> : <div key={row.item.id} style={{ ...shellCardStyle, padding: 10, borderRadius: 14, boxShadow: 'none' }}><div style={{ fontSize: 12, fontWeight: 800 }}>{getTransactionLabel(row.item.transaction_type)}</div><div style={{ fontSize: 12, color: 'var(--muted, #64748b)', marginTop: 4 }}>{formatTradeDate(row.item.transaction_date)} · {formatCurrency(Number(row.item.amount))}</div><div className="ab-row-gap" style={{ marginTop: 8 }}><button type="button" className="ab-btn ab-btn-subtle" onClick={() => editCash(row.item)} style={primaryBtnStyle}>Sửa</button><button type="button" className="ab-delete ghost" onClick={() => deleteCash(row.item)}>Xóa</button></div></div>) : <div className="ab-note">Chưa có lịch sử giao dịch</div>}</div>
        </Section>
