  ...
  return (
    <div style={{ padding: 14, border: '1px solid var(--border)', borderRadius: 16, background: 'var(--soft)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
        {[
          { label: 'ENTRY', value: pick.entry, title: 'Vùng giá nên mua vào', style: { background: 'var(--card)',         border: '1px solid var(--border)',       color: 'var(--text)'  } },
          { label: 'TP',    value: pick.tp,    title: 'Chốt lời — mục tiêu giá AI đề xuất dựa trên kháng cự kỹ thuật', style: { background: 'var(--green-surface)', border: '1px solid var(--green-border)', color: 'var(--green)' } },
          { label: 'SL',    value: pick.sl,    title: 'Cắt lỗ — ngưỡng giá nên thoát để bảo vệ vốn', style: { background: 'var(--red-surface)',   border: '1px solid var(--red-border)',   color: 'var(--red)'   } },
        ].map(({ label, value, title, style }) => (
          <div key={label} title={title} style={{ padding: '6px 0', borderRadius: 10, textAlign: 'center', cursor: 'help', ...style }}>
            <div style={{ fontSize: 9, fontWeight: 800, color: style.color }}>{label}</div>
            <div className="num-premium" style={{ fontWeight: 800, fontSize: 13, marginTop: 2, color: style.color }}>
              {formatPrice(value)}
            </div>
          </div>
        ))}
