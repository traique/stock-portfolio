  ...
  return (
    <div style={S.page}>
      ...
      <div style={S.card}>
        ...
        <button style={{ ...S.btn('#0ea5e9'), marginBottom: 8 }} onClick={async () => {
          addLog('Test Yahoo qua Next.js API route...', 'info');
          try {
            const r = await fetch('/api/test-yahoo?sym=BID');
            const d = await r.json();
            if (d.ok) {
              addLog(`✅ Yahoo hoạt động từ Vercel! closes=${d.closes_count} last=${d.last_close}`, 'ok');
            } else {
              addLog(`❌ Yahoo fail: ${d.error}`, 'err');
            }
          } catch(e) {
            addLog(`❌ ${e instanceof Error ? e.message : String(e)}`, 'err');
          }
        }}>
          🌐 TEST YAHOO QUA VERCEL API
        </button>
        <button style={{ ...S.btn('#64748b'), marginBottom: 8 }} onClick={() => {
          addLog(`Edge URL: ${EDGE_URL}`, 'info');
          addLog(`Anon key: ${SUPABASE_ANON ? SUPABASE_ANON.slice(0,20) + '...' : '❌ EMPTY'}`, 'info');
          addLog(`Supabase URL: ${SUPABASE_URL || '❌ EMPTY'}`, 'info');
        }}>
          🔎 SHOW CONFIG
        </button>
        <button style={{ ...S.btn('#6366f1'), marginBottom: 8 }} onClick={async () => {
        ...
        <button style={{ ...S.btn('#8b5cf6'), marginBottom: 0 }} onClick={async () => {
          ...
          try {
            ...
            d.results?.forEach((ep: {name: string; status: number; ok: boolean; preview?: string; error?: string}) => {
              if (ep.ok) {
                addLog(`✅ ${ep.name} — HTTP ${ep.status} — ${ep.preview?.slice(0,150)}`, 'ok');
              } else {
                addLog(`❌ ${ep.name} — HTTP ${ep.status}${ep.error ? ' — ' + ep.error : ''} | body: ${ep.preview?.slice(0,150) ?? ''}`, 'err');
              }
            });
          } catch(e) {
