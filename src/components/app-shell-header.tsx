...
export default function AppShellHeader({
  ...
  // Model list — fetched live từ /api/ai/models
  ...
  // --- FETCH MODEL LIST khi mở dropdown lần đầu ---
  const fetchModels = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const models: AiModelMeta[] = data.models ?? [];
        setAiModels(models);

        // Nếu model đang lưu không còn trong list → reset về default
        const savedModel = localStorage.getItem(AI_MODEL_KEY);
        if (savedModel && !models.some(m => m.key === savedModel)) {
          setAiModelState(DEFAULT_MODEL);
          localStorage.setItem(AI_MODEL_KEY, DEFAULT_MODEL);
        }
      }
    } catch {
      // giữ nguyên list rỗng, UI hiện skeleton
    } finally {
  ...
  const handleMenuOpen = (e: React.MouseEvent) => {
    ...
    setMenuOpen(opening);
    if (opening) fetchModels(); // lazy-load models khi mở menu
  };
      ...
      <div style={TOP_ROW_STYLE}>
        ...
        <div style={ACTIONS_STYLE}>
          ...
          <div ref={menuRef} style={{ position: 'relative' }}>
            {isLoggedIn ? (
              <button onClick={handleMenuOpen} style={ACCOUNT_BTN_STYLE}>
            ...
            {menuOpen && isLoggedIn && (
              <div style={{ ...DROPDOWN_STYLE, minWidth: 240 }}>
                {/* User info */}
                ...
                <div style={{ marginBottom: 12 }}>
                  ...
                  {/* Scrollable list — giới hạn chiều cao để không tràn màn hình */}
                  <div style={{
                    maxHeight: '55vh',
                    overflowY: 'auto',
                    overflowX: 'hidden',
                    ...
                    {modelsLoading ? (
                      [1, 2, 3].map(i => (
                        <div key={i} style={{
                          height: 48, borderRadius: 10, marginBottom: 2,
                          background: 'var(--soft)', opacity: 0.6,
                        }} />
                      ))
                    ) : (
                      (['gemini', 'groq'] as const).map(provider => {
                        ...
                        return (
                          <div key={provider}>
                            ...
                            {group.map(m => {
                              const isSelected = aiModel === m.key;
                              return (
                                <button
                                  key={m.key}
                                  type="button"
                                  ...
                                  style={{
                                    ...
                                    border: isSelected ? '1px solid rgba(59,130,246,0.40)' : '1px solid transparent',
                                ...
                                >
                                  <div style={{ minWidth: 0 }}>
                                    <div style={{ fontSize: 12, fontWeight: 800, color: 'var(--text)', display: 'flex', alignItems: 'center', gap: 5, flexWrap: 'wrap' }}>
                                      {m.label}
                                      <span style={{
                                        fontSize: 9, fontWeight: 800, padding: '1px 5px',
                                        borderRadius: 99, flexShrink: 0,
                                        background: provider === 'gemini' ? 'rgba(59,130,246,0.15)' : 'rgba(16,185,129,0.15)',
                                  ...
                                  {isSelected && <Check size={14} color="#3b82f6" style={{ flexShrink: 0 }} />}
                                </button>
                              );
                            })}
                          </div>
