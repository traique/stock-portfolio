...
type Props = {
  email?: string;
  isLoggedIn: boolean;
  currentTab: 'home' | 'dashboard' | 'market' | 'gold' | 'oil' | 'system-live' | 'backtest';
  onLogout?: () => void;
  onAuthOpen?: () => void;
};
...
/* ── Glass pill — style động (vị trí/size theo từng nơi dùng); màu/nền lấy từ
   class .ab-glass-pill / .ab-icon-btn / .ab-account-btn / .ab-dropdown (CSS) ── */
...
export default function AppShellHeader({
  ...
  return (
    <header className="ab-header-shell">
      {/* Specular glow */}
      <div className="ab-header-glow" />
          ...
          <div ref={menuRef} style={ { position: 'relative' } }>
            {isLoggedIn ? (
              <button
                ...
                aria-haspopup="menu"
                style={ { maxWidth: 120, overflow: 'hidden', flexShrink: 0 } }
              >
              ...
              <button onClick={e => { e.stopPropagation(); onAuthOpen?.(); }} className="ab-account-btn" style={ { flexShrink: 0 } }>
                SIGN IN
              </button>
            )}

            {menuOpen && isLoggedIn && (
              <div className="ab-dropdown" role="menu" style={ { right: 0, width: 262 } }>
      ...
      <div style={ { position: 'relative', display: 'flex', alignItems: 'center', gap: 6 } }>
        ...
        <nav style={ { display: 'flex', gap: 2, alignItems: 'center' } } aria-label="Điều hướng chính">
          <Link href="/"          aria-current={currentTab === 'home' ? 'page' : undefined}          style={tabStyle(currentTab === 'home')}>HOME</Link>
          <Link href="/dashboard" aria-current={currentTab === 'dashboard' ? 'page' : undefined}    style={tabStyle(currentTab === 'dashboard')}>DANH MỤC</Link>
          ...
          <div ref={toolsRef} style={ { position: 'relative' } }>
            ...
            {toolsOpen && (
              <div className="ab-dropdown" role="menu" style={ { left: 0, minWidth: 156 } }>
                {[
                  ...
                  <Link
                    ...
                    href={item.href}
                    role="menuitem"
                    aria-current={currentTab === item.href.slice(1) ? 'page' : undefined}
