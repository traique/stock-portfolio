...
// recharts (~100KB gz kèm d3) là viên nặng nhất của dashboard — tách thành
// async chunk, chỉ tải khi chart mount (chart nằm dưới fold + tự fetch data
// riêng nên không cần nằm trong first-load JS).
const PerformanceChart = dynamic(
  () => import('./performance-chart').then(m => ({ default: m.PerformanceChart })),
  {
    ssr: false,
    loading: () => (
...
const NewsModal = memo(function NewsModal({ symbol, news, onClose }: {
  ...
  return (
    <Modal open onClose={onClose} ariaLabel={`Tin tức ${symbol}`} maxWidth={620}>
