...
// Màu phải dùng CSS token (không hardcode trắng/đen) để chart đúng ở CẢ light & dark mode.
const TOOLTIP_CURSOR = { stroke: 'var(--border-strong)', strokeWidth: 1, strokeDasharray: '4 4' };
...
export const PerformanceChart = memo(function PerformanceChart({ accessToken }: Props) {
  ...
  const [yMin, yMax] = useMemo(() => {
    const useIdx = showCompare && hasVnData;
    const vals = chartData
      .map(p => (useIdx ? p.nav_indexed : p.total_assets))
  ...
  const legendItems = [
    ...
    { color: 'var(--muted)', label: 'Vốn gốc', value: lastPoint ? fmtCurrency(lastPoint.net_capital) : '—' },
  ];
        ...
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={chartData} margin={chartMargin}>
            ...
            {!showCompare && (lastPoint?.net_capital ?? 0) > 0 && (
              <ReferenceLine y={lastPoint!.net_capital} stroke="var(--border-strong)" strokeDasharray="5 5" strokeWidth={1} />
            )}

            {showCompare && hasVnData && (
              <ReferenceLine y={100} stroke="var(--border-strong)" strokeDasharray="5 5" strokeWidth={1} />
            )}
