...
export function PerformanceChart({ accessToken }: Props) {
  ...
  const yMin = useMemo(() => {
    ...
    // Use only total_assets for domain — net_capital can be much higher
    // and would compress the visible line to near-flat
    const min = Math.min(...snapshots.map(s => s.total_assets));
