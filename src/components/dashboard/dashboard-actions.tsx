...
const fmtDate  = (v?: string | null) => !v ? 'Không ngày' : dateFmt.format(new Date(v));

function fmtInt(v: string): string {
  const d = v.replace(/[^\d]/g, '');
  return d ? Number(d).toLocaleString('en-US') : '';
}
function parseInt2(v: string): number { return Number(v.replace(/[^\d]/g, '')) || 0; }

function txLabel(type: string): string {
  const m: Record<string, string> = { BUY: 'Mua', SELL: 'Bán', DEPOSIT: 'Nạp tiền', WITHDRAW: 'Rút tiền' };
...
function RiskSelector({ value, onChange }: { value: RiskProfile; onChange: (v: RiskProfile) => void }) {
  ...
  return (
    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
      {opts.map(o => (
        <button key={o.v} type="button" className={`ab-risk-btn${value === o.v ? ' active' : ''}`} onClick={() => onChange(o.v)}>
          {o.label}
        </button>
      ))}
    </div>
  ...
  return (
    <div style={{ padding: 14, border: '1px solid var(--border)', borderRadius: 16, background: 'var(--card)', display: 'flex', flexDirection: 'column', gap: 10 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <span style={{ fontSize: 11, fontWeight: 800, padding: '4px 10px', borderRadius: 99, background: 'var(--soft)', color: C_MUTED }}>{item.confidence}</span>
      </div>
...
export function DashboardActions({
  ...
  const historyRows = useMemo<HistoryRow[]>(() => {
    ...
    // Cash rows only shown when no symbol filter is active
              ...
              {row.kind === 'trade' ? (
                <>
                  ...
                  <div className="num-premium" style={{ fontSize: 12, color: C_MUTED, marginTop: 6 }}>
                    ...
                    {row.item.transaction_type === 'SELL' ? ` · CHỐT ${formatCurrency(Number((row.item as EnrichedTx).realized_pnl ?? 0))}` : ''}
                  </div>
                </>
              ) : (
                <>
      ...
      <Section kicker="AI Assistant" title="NHẬN XÉT & CHIẾN LƯỢC" open={aiOpen} onToggle={() => setAiOpen(v => !v)}>
        ...
        {aiResult ? (
          ...
          <div style={{ marginTop: 10, color: C_MUTED, fontSize: 13 }}>
            Nhấn "Phân tích danh mục" để AI đưa gợi ý xử lý và thiết lập TP/SL theo khẩu vị rủi ro của bạn.
          </div>
        )}
      </Section>
