...
async function getSupaCached<T>(cacheKey: string): Promise<T | null> {
  try {
    ...
    logger.warn('[price-cache] L2 read thất bại', { cacheKey, err: String(e) });
    return null;
  }
}
...
async function setSupaCached<T>(cacheKey: string, value: T, ttlMs: number): Promise<void> {
  try {
    ...
    // Cache write failure không làm hỏng main flow — chỉ log để rà soát
    logger.warn('[price-cache] L2 write thất bại', { cacheKey, err: String(e) });
  }
}
