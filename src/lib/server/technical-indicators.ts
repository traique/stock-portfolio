...
// Lesson 2: Support / Resistance từ OHLCV thật (high/low 30 phiên)
export type SupportResistance = {
  support:    number | null;  // min(low) trong lookback phiên
  resistance: number | null;  // max(high) trong lookback phiên
  distToSupport:    number;   // % giá cách support (âm = dưới support)
  distToResistance: number;   // % giá cách resistance (âm = trên resistance)
};
...
export function calcSupportResistance(
  ...
  if (len === 0) return { support: null, resistance: null, distToSupport: 0, distToResistance: 0 };
  ...
  const distToSupport    = support    > 0 ? Number(((price - support)    / support    * 100).toFixed(1)) : 0;
  const distToResistance = resistance > 0 ? Number(((price - resistance) / resistance * 100).toFixed(1)) : 0;
...
// Lesson 4: Bias MA — đo độ lệch giá so với SMA (%)
export type BiasMA = {
  bias:   number;   // (price - ma) / ma * 100
  status: 'nguy_hiem' | 'canh_giac' | 'an_toan' | 'chiet_khau' | 'qua_ban';
};

export function calcBiasMA(price: number, ma: number): BiasMA {
  if (ma <= 0) return { bias: 0, status: 'an_toan' };
  const bias = Number(((price - ma) / ma * 100).toFixed(2));

  let status: BiasMA['status'];
  if      (bias >  8)  status = 'nguy_hiem';
  else if (bias >  5)  status = 'canh_giac';
  else if (bias < -8)  status = 'qua_ban';
  else if (bias < -5)  status = 'chiet_khau';
  else                 status = 'an_toan';

  return { bias, status };
}

// Lesson 5: MA alignment (MA5 > MA10 > MA20) + trendScore tổng hợp
...
export function calcMAAlignment(closes: number[]): MAAlignment {
  if (closes.length < 20) {
    return { ma5: 0, ma10: 0, ma20: 0, alignment: 'unknown', isBullish: null };
  }
  ...
  let alignment: MAAlignment['alignment'] = 'mixed';
  let isBullish: boolean | null = null;

  if (ma5 > ma10 && ma10 > ma20) { alignment = 'bullish'; isBullish = true; }
  else if (ma5 < ma10 && ma10 < ma20) { alignment = 'bearish'; isBullish = false; }

  return {
    ma5:  Number(ma5.toFixed(0)),
    ma10: Number(ma10.toFixed(0)),
    ma20: Number(ma20.toFixed(0)),
    alignment,
    isBullish,
  };
}

// Tổng hợp trendScore từ MA alignment + RSI + MACD (giống mozy)
export function calcTrendScore(
  ...
  // MA alignment: ±20
  if (maAlign.alignment === 'bullish') score += 20;
  else if (maAlign.alignment === 'bearish') score -= 20;
  else if (maAlign.ma5 > maAlign.ma10 || maAlign.ma10 > maAlign.ma20) score += 5;
  else score -= 5;
