...
const globalForPriceCache = globalThis as typeof globalThis & {
  __priceCacheStore__?: Map<string, PriceCacheEntry<unknown>>;
};

function getStore() {
  if (!globalForPriceCache.__priceCacheStore__) {
    globalForPriceCache.__priceCacheStore__ = new Map();
  }
  return globalForPriceCache.__priceCacheStore__;
}
