...
// VCI Edge realtime (fallback) cho HOSE/HNX/UPCOM
import { getVciEdgeBatch } from './providers/vci-edge';

// ✨ DNSE realtime — nguồn CHÍNH (trễ ~1 phút), đủ 3 sàn + VNINDEX
...
export async function fetchMarketPrices(symbols: string[]): Promise<PricesPayload> {
	// ── Bước 1: DNSE realtime — NGUỒN CHÍNH (trễ ~1 phút, đủ 3 sàn + VNINDEX) ──
	const dnseMap = symbols.length
		? await getDnseEdgeBatch(symbols).catch(err => {
				console.error('[DNSE Fail]', err);
				Sentry.captureException(err, {
					tags: { module: 'market', stage: 'dnse' },
					extra: { symbols },
				});
	...
	const missedAfterDnse = symbols.filter(s => !hasValidPrice(dnseMap.get(s)));

	// ── Bước 2: Yahoo (fallback 1 — trễ ~5 phút) cho mã DNSE thiếu ──────────
	const yahooSettled = await Promise.allSettled(
		missedAfterDnse.map(s => getYahooMarketData(s)),
	);
	const yahooMap = new Map<string, MarketData>();
	yahooSettled.forEach((r, i) => {
		const sym = missedAfterDnse[i];
		if (r.status === 'rejected') {
			console.warn(`[Yahoo Fail] ${sym}:`, r.reason);
		} else if (hasValidPrice(r.value)) {
			yahooMap.set(sym, r.value);
		}
	});

	const missedAfterYahoo = missedAfterDnse.filter(s => !hasValidPrice(yahooMap.get(s)));

	// ── Bước 3: VCI Edge (fallback 2 — trễ ~30 phút) ───────────────────────
	...
	// ── Bước 4: snapshot DB (fallback cuối, do cron bơm) ───────────────────
	...
	// ── Bước 5: gộp theo ưu tiên DNSE → Yahoo → VCI Edge → snapshot ────────
	let dnseOk = 0;
	let yahooOk = 0;
	let vciOk = 0;
	let snapshotOk = 0;
	...
	const results: MarketData[] = symbols.map((symbol) => {
		const dnse = dnseMap.get(symbol);
		if (hasValidPrice(dnse)) {
			dnseOk += 1;
			return dnse;
		}

		const yahoo = yahooMap.get(symbol);
		if (hasValidPrice(yahoo)) {
			yahooOk += 1;
			return yahoo;
		}
	...
	return {
		...
		provider: 'dnse+yahoo+vci+snapshot',
		debug: results,
		health,
	};
    }
