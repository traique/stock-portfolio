...
import { getHistory } from '@/lib/server/history-store';
...
type PriceHistory = {
	...
	dates?: string[]; // ngày thật của từng nến
};
...
// ⚠️ ĐỐI CHIẾU KHỐI NÀY với file gốc của bạn (regex bị tool tải làm méo).
...
const HISTORY_CONCURRENCY = 5; // tránh fire hàng chục mã song song
...
// ================= HISTORY — 1 NGUỒN DUY NHẤT (history-store) =================
// DB-first (price_history) + tự backfill DNSE khi thiếu, service role.
// Đơn vị VND thô đồng nhất cho HOSE / HNX / UPCOM / VNINDEX.
// Không còn rẽ nhánh Yahoo / VCI chart / Supabase-anon.
...
async function fetchHistory(symbol: string): Promise<PriceHistory> {
	try {
		const rows = await getHistory(symbol, 90);
		if (!rows.length) return EMPTY_HISTORY;
		...
		console.error(`[fetchHistory] getHistory failed for ${symbol}:`, err);
...
export async function buildTechnicalSignals(
	...
	// ✨ Giới hạn concurrency để tránh quá tải DB/backfill
