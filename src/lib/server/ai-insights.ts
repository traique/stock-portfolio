...
async function fetchYahooHistory(ticker: string): Promise<PriceHistory> {
  ...
  for (const host of YAHOO_HOSTS) {
    try {
      ...
      const high: number[] = (q.high ?? [])  // ✨ mozy lesson 2
        .map(Number)
        .filter((v: number) => Number.isFinite(v) && v > 0);

      const low: number[] = (q.low ?? [])    // ✨ mozy lesson 2
        .map(Number)
        .filter((v: number) => Number.isFinite(v) && v > 0);
...
// ─── Save to Supabase after Yahoo fetch ──────────────────────────────────────
async function saveHistoryToSupabase(symbol: string, exchange: string, history: PriceHistory): Promise<void> {
  try {
    ...
    const rows = history.close.map((close, i) => {
      ...
      return {
        ...
        open:   history.high[i] != null && history.low[i] != null
          ? Math.round((history.high[i] + history.low[i]) / 2)
          : close,  // Yahoo không có open riêng, dùng midpoint(high, low) làm proxy
  ...
  } catch { /* non-critical, không block */ }
}
...
async function fetchHistory(symbol: string): Promise<PriceHistory> {
  ...
  // Bước 1: Supabase price_history — nhanh, có tất cả sàn nếu đã được lưu
  try {
    const hist = await fetchSupabaseHistory(symbol, 90);
    if (hist.close.length >= 20) return hist;
  } catch { /* fallthrough */ }

  // Bước 2: HOSE → Yahoo (hoạt động từ Vercel) → tự động lưu vào Supabase
  if (exchange !== 'HNX' && exchange !== 'UPCOM') {
    try {
      const hist = await fetchYahooHistory(`${symbol}.VN`);
      if (hist.close.length > 0) {
        // Fire-and-forget: lưu vào Supabase để lần sau dùng luôn
