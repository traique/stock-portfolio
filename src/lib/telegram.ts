...
// ⚠️ FIX: cron Vercel chỉ chạy 1 lần/ngày ở giờ CỐ ĐỊNH (08:10 UTC = 15:10 VN).
// Cách cũ yêu cầu now.getUTCHours() === daily_hour_utc nên gần như không bao giờ
// khớp ⇒ cron không gửi (dù nút Test vẫn gửi được vì không đi qua hàm này).
// Nay BỎ khóa theo giờ, chỉ chống gửi TRÙNG trong cùng một ngày (theo lịch VN).
export function shouldSendDaily(
  lastDailySentAt: string | null,
  now: Date,
  _dailyHourUtc?: number,
) {
  void _dailyHourUtc; // giữ tham số để không phải sửa nơi gọi
