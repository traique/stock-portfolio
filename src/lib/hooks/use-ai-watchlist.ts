...
export function useAiWatchlist(
  ...
  useEffect(() => {
    ...
    if (!saved) return;
    try { setAiWatchlist(JSON.parse(saved)); } catch { /* cache local hỏng → bỏ qua, quét mới */ }
  }, [userId]);
  ...
  const fetchScan = useCallback(async (forceEod = false) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        toast.error(payload?.error ?? 'Không thể phân tích watchlist');
      } else {
        setAiWatchlist(payload);
        if (mode === 'eod') {
          const n = payload.picks?.length ?? 0;
          toast.success(
            payload.cached
              ? `Đã tải phân tích AI (${n} mã, bản cache)`
              : `AI vừa phân tích xong ${n} mã trong danh sách`,
          );
        }
      }
    } catch {
