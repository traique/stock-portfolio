...
export function useWatchlist({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (isLoggedIn && userId) {
        ...
        } catch { /* DB lỗi → rơi xuống localStorage/fallback dưới, không phá UI */ }
      }
      ...
      if (saved) {
        ...
        } catch { /* localStorage hỏng/chặn → dùng default watchlist */ }
      }
  ...
  useEffect(() => {
    ...
    (async () => {
      if (isLoggedIn && userId) {
        ...
        } catch { /* persist lỗi (offline/private mode) — giữ trạng thái trong memory */ }
      }
  ...
  const removeSymbol = useCallback((symbol: string) => {
    ...
    toast.info(`Đã xoá ${symbol} khỏi danh sách`);
  }, []);
