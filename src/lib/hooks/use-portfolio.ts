...
import {
  ...
  CashSummaryShape, DashboardInitialData, PricesResponse, QuoteItem,
...
// ── Hook ─────────────────────────────────────────────────────────────────────
/**
 * @param initial Dữ liệu được Server Component prefetch sẵn (auth + 3 bảng).
 *                Khi có initial: bỏ qua waterfall session→DB ở client, dữ liệu
 *                có ngay từ render đầu; chỉ còn fetch giá chạy song song.
 */
export function usePortfolio(initial?: DashboardInitialData): UsePortfolioReturn {
  // Auth
  ...
  const [cashTransactions, setCashTransactions] = useState<CashTransaction[]>(initial?.cashTransactions ?? []);
  const [portfolioSettings, setPortfolioSettings] = useState<PortfolioSettings | null>(initial?.portfolioSettings ?? null);
  ...
  const [message, setMessage] = useState(initial?.loadError ?? '');
  // AI cache
  const [aiResult, setAiResult] = useState<AiPortfolioResponse | null>(null);

  // Guard: chỉ chạy init một lần (các callback là stable nhưng tránh re-run khi dev StrictMode)
  ...
  useEffect(() => {
    ...
    if (initial) {
      // Server Component đã xác thực + nạp dữ liệu → client chỉ cần access token
      // (đọc session cục bộ, không gọi mạng) cho các API call (AI, export...).
      let cancelled = false;
      (async () => {
  ...
  }, [bootstrapSession, loadPortfolio, loadVnIndex, initial]);

  // ── AI cache — persist/restore ────────────────────────────────────────────
  useEffect(() => {
    if (!userId) return;
    const saved = localStorage.getItem(getAiPortfolioKey(userId));
    if (saved) { try { setAiResult(JSON.parse(saved)); } catch { /* cache local hỏng → bỏ qua */ } }
  }, [userId]);
