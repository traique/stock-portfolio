...
/**
 * Dữ liệu dashboard được Server Component prefetch (auth + 3 bảng) và truyền
 * xuống client. Giúp bỏ 2 chặng waterfall đầu tiên (auth.getUser → query DB)
 * — client có dữ liệu ngay từ render đầu tiên, chỉ còn fetch giá chạy song song.
 */
export type DashboardInitialData = {
  ...
  /** Lỗi query phía server (nếu có) — hiển thị thay vì im lặng */
  loadError?: string;
};
...
export type CashSummaryShape = {
  ...
  actualCash:     number;
  netCapital:     number;
  dividends:      number;   // ✨ tổng cổ tức tiền mặt đã nhận
};
...
export const DEFAULT_CASH_FORM = {
  transaction_type: 'DEPOSIT' as 'DEPOSIT' | 'WITHDRAW' | 'DIVIDEND',
  amount: '', transaction_date: '', note: '',
};

// Scoped per-user to prevent AI result leakage between accounts on shared devices
export const getAiPortfolioKey = (userId?: string) =>
  `lcta_ai_portfolio_${userId ?? 'guest'}`;
