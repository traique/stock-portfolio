...
--    ⚠️ BẢO MẬT: KHÔNG hardcode service_role key trong SQL!
--    Key cũ đã bị lộ trong repo — PHẢI rotate trong Dashboard → Settings → API.
--    Cách đúng: lưu key vào Supabase Vault 1 lần:
--      select vault.create_secret(
--        '<SERVICE_ROLE_KEY_MỚI_SAU_KHI_ROTATE>', 'service_role_key'
--      );
--    Function bên dưới đọc từ Vault lúc runtime, không lưu key trong source.
-- =============================================================
...
begin
  ...
  -- Đọc service key từ Vault (tạo secret với tên 'service_role_key' như hướng dẫn trên).
  -- Nếu chưa tạo secret → cảnh báo rõ ràng thay vì gửi request không auth.
  begin
  ...
  if service_key is null or service_key = '' then
    raise warning 'fetch_vn_prices: chưa có secret ''service_role_key'' trong Vault — bỏ qua lần chạy này.';
    return;
  end if;
