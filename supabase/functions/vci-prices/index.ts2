...
// Giải pháp: gọi Vercel (Washington DC) làm proxy → Vercel fetch Yahoo OK.
//
// Flow: Edge Function (Singapore) → lcta.vercel.app/api/history/[sym] → Yahoo
...
async function fetchOHLCV(symbol: string, days = 90): Promise<any[]> {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new Error(`Vercel proxy ${symbol} HTTP ${res.status} — ${body.slice(0, 100)}`);
    }
...
serve(async (req) => {
  ...
  try {
    ...
    if (mode === 'eod') {
      ...
      // Pass 1: Lấy VCI realtime snapshot cho TẤT CẢ symbol (kể cả HNX/UPCOM).
      // Dùng làm fallback OHLCV khi Yahoo không có data (HNX/UPCOM không có trên Yahoo).
      // deno-lint-ignore no-explicit-any
      ...
      // Pass 2: Lưu OHLCV — Yahoo qua Vercel proxy cho HOSE, VCI realtime cho HNX/UPCOM
      const settled = await Promise.allSettled(
        allSymbols.map(async (sym: string) => {
          const exchange = exchangeMap[sym] ?? 'HOSE';

          // Thử Yahoo qua Vercel proxy (hoạt động cho HOSE, trả [] cho HNX/UPCOM)
          let candles = await fetchVciOHLCV(sym, days).catch(() => []);

          if (!candles.length) {
            // HNX/UPCOM hoặc Yahoo fail → dùng VCI realtime snapshot làm 1 row EOD
      ...
      settled.forEach((r, i) => {
        if (r.status === 'fulfilled') {
          // deno-lint-ignore no-explicit-any
          if ((r.value as any).saved === 0) skipped++;
          else success++;
        } else {
          failed++;
